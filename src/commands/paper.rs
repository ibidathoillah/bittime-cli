...
const DEFAULT_PAIR: &str = "USDTIDR";
...
impl Default for PaperState {
    fn default() -> Self {
        ...
        balances.insert("USDT".to_string(), DEFAULT_BASE_BALANCE);
        Self {
            balances,
            orders: Vec::new(),
            next_order_id: 1,
...
impl PaperState {
    fn load() -> Result<Self, BittimeError> {
        let path = Config::paper_state_path()?;
        if !path.exists() {
            return Ok(Self::default());
        }
        let content = fs::read_to_string(&path).map_err(|e| {
            BittimeError::Config(format!("Failed to read {}: {}", path.display(), e))
        })?;
        serde_json::from_str(&content).map_err(BittimeError::from)
    }

    fn save(&self) -> Result<(), BittimeError> {
        let path = Config::paper_state_path()?;
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)?;
        }
        let content = serde_json::to_string_pretty(self).map_err(BittimeError::from)?;
    ...
    fn cancel_order(&mut self, order_id: u64) -> Result<(), BittimeError> {
        let order = self
            .orders
            .iter_mut()
            .find(|order| order.id == order_id)
            .ok_or_else(|| {
                BittimeError::Validation(format!("Paper order {} not found", order_id))
            })?;
        if order.status != "OPEN" {
            return Err(BittimeError::Validation(format!(
                "Paper order {} is {}",
    ...
    fn cancel_all(&mut self, pair: Option<&str>) -> usize {
        ...
        for order in &mut self.orders {
            if order.status == "OPEN" && pair.as_ref().is_none_or(|p| p == &order.pair) {
                order.status = "CANCELED".to_string();
                count += 1;
            }
        }
    ...
    fn fill_order(&mut self, order_id: u64, price: Option<f64>) -> Result<(), BittimeError> {
        let idx = self
            .orders
            .iter()
        ...
        let fill_price = price.or(self.orders[idx].price).ok_or_else(|| {
            BittimeError::Validation("Fill price is required for market paper orders".to_string())
        })?;
        ...
        match side.as_str() {
            ...
            _ => {
                return Err(BittimeError::Internal(format!(
                    "Invalid paper side {}",
                    side
                )))
            }
    ...
    fn orders_output(
        ...
        let mut orders: Vec<&PaperOrder> = self
            ...
            .filter(|order| pair.as_ref().is_none_or(|p| p == &order.pair))
            .collect();
...
fn split_pair(pair: &str) -> Result<(String, String), BittimeError> {
    let normalized = normalize_pair(pair);
    for quote in ["IDR", "USDT", "BTC", "ETH", "BNB"] {
    ...
    Err(BittimeError::Validation(format!(
        "Cannot infer base/quote assets from pair {}",
        pair
    )))
}

fn validate_positive(value: f64, name: &str) -> Result<(), BittimeError> {
...
mod tests {
    ...
    fn ctx() -> AppContext {
        AppContext {
            client: crate::client::BittimeClient::new("https://example.com", None),
    ...
    async fn test_place_and_fill_buy() {
        let mut state = PaperState::for_pair("USDTIDR", Some(20_000.0), Some(0.0)).unwrap();
        let id = state
            .place_order("USDTIDR", "BUY", Some(10_000.0), 1.0)
            .unwrap();
        state.fill_order(id, None).unwrap();
        assert_eq!(state.balances["USDT"], 1.0);
