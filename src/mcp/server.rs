...
pub struct BittimeMcpService {
    client: BittimeClient,
    registry: ToolRegistry,
}

impl BittimeMcpService {
    pub fn new(client: BittimeClient, _config: Config) -> Self {
        Self {
            client,
...
impl ServerHandler for BittimeMcpService {
...
pub async fn run(client: BittimeClient, config: Config) -> Result<()> {
    let service = BittimeMcpService::new(client, config);
...
mod tests {
    ...
    async fn test_mcp_get_info() {
        let client = BittimeClient::new("h".into(), None);
        let service = BittimeMcpService::new(client, Config::default());
        let info = service.get_info();
        assert_eq!(info.server_info.name, "bittime-cli");
    }
